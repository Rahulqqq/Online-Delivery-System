...
public class DynamboDBLocal {
	...
	private void createTable(DynamoDB dynamoDB) {
		...
		keySchema.add(new KeySchemaElement().withAttributeName("paymentId").withKeyType(KeyType.HASH));

		ArrayList<AttributeDefinition> attributeDefinitions = new ArrayList<>();
		attributeDefinitions.add(new AttributeDefinition().withAttributeName("paymentId").withAttributeType("S"));

		CreateTableRequest request = new CreateTableRequest().withTableName("Payments").withKeySchema(keySchema)
